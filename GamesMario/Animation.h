...
{
public:
	std::vector<IntRect> frames, frames_flip;  /// кадр и обратный кадр (движение в обратную сторону)
	float currentFrame, speed;            //// количество кадров и скорость проигрывания анимации	
	// loop показвает зациклена ли анимация. 
	//анимация взрыва, смерти должна проиграться один раз и остановиться, loop=false
	...
	void tick(float time)   // функция смены кадра
	{
		if (!isPlaying) return;
	...
	void loadFromXML(std::string fileName, Texture &t)
	{
		...
		while (animElement)
		{
			...
			while (cut)
			{
				...
				anim.frames_flip.push_back(IntRect(x, y, -w, h));
				cut = cut->NextSiblingElement("cut");			 
			}
			
			anim.sprite.setOrigin (0, anim.frames[0].height);
			animList[currentAnim] = anim;     // массив анимаций
